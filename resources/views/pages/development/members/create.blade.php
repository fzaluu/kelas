...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center space-x-3">
        ...
        <div class="min-w-0">
            <h1 class="text-xl font-bold text-slate-900 truncate">Tambah Anggota Kelas Baru</h1>
            <p class="text-xs text-slate-500">Masukkan biodata siswa kelas XI PPLG 2.</p>
        </div>
    </div>

    <form action="{{ route('development.members.store') }}" method="POST" class="bg-white p-5 sm:p-8 rounded-2xl border border-slate-200/80 shadow-sm space-y-4">
        @csrf
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div class="space-y-1">
                ...
                <input type="text" name="nis" value="{{ old('nis') }}" placeholder="Contoh: 22231001" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-xs focus:ring-2 focus:ring-blue-600 focus:outline-none" required>
            ...
            <div class="space-y-1">
                ...
                <input type="text" name="nisn" value="{{ old('nisn') }}" placeholder="Contoh: 0061234567" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-xs focus:ring-2 focus:ring-blue-600 focus:outline-none">
        ...
        <div class="space-y-1">
            ...
            <input type="text" name="full_name" value="{{ old('full_name') }}" placeholder="Nama sesuai dokumen resmi" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-xs focus:ring-2 focus:ring-blue-600 focus:outline-none" required>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <!-- 🔍 HUBUNGKAN KE AKUN (MODULAR REUSABLE LIVE SEARCH DROPDOWN) -->
            <div class="space-y-1 relative" data-live-search="true">
                ...
                <input type="hidden" name="user_id" class="select-real-input" value="{{ old('user_id') }}">
                ...
                <div class="select-trigger-btn w-full px-4 py-2.5 rounded-xl border border-slate-200 text-xs bg-white cursor-pointer flex items-center justify-between shadow-sm hover:border-blue-400 transition">
                    <span class="select-display-text truncate font-medium text-slate-800">-- Tanpa Akun --</span>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div class="space-y-1">
                ...
                <input type="text" name="pob" value="{{ old('pob') }}" placeholder="Contoh: Tasikmalaya" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-xs focus:ring-2 focus:ring-blue-600 focus:outline-none">
            </div>

            <!-- Tanggal Lahir -->
            <div class="space-y-1">
                <label class="text-xs font-bold text-slate-700 uppercase tracking-wider">Tanggal Lahir</label>
                <input type="date" name="dob" value="{{ old('dob') }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-xs focus:ring-2 focus:ring-blue-600 focus:outline-none">
            </div>
        </div>
        ...
        <div class="space-y-1">
            ...
            <textarea name="address" rows="2" placeholder="Alamat domisili siswa..." class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-xs focus:ring-2 focus:ring-blue-600 focus:outline-none">{{ old('address') }}</textarea>
        </div>
        ...
        <div class="space-y-1">
            ...
            <select name="status" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-xs focus:ring-2 focus:ring-blue-600 focus:outline-none bg-white" required>
                ...
                <option value="TRANSFERRED">TRANSFERRED (Pindah)</option>
                <option value="DROPPED_OUT">DROPPED_OUT (Keluar)</option>
            </select>
        </div>
