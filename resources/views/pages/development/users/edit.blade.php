...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center space-x-3">
        ...
        <div>
            ...
            <p class="text-xs text-slate-500">Ubah kredensial atau hak akses pengguna <span class="font-bold text-slate-700">{{ $user->username }}</span>.</p>
        </div>
    </div>
    ...
    <form action="{{ route('development.users.update', $user->id) }}" method="POST" class="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200/80 shadow-sm space-y-5">
        ...
        <div class="space-y-1">
            ...
            <input type="text" name="name" value="{{ old('name', $user->name) }}" placeholder="Contoh: Budi Santoso" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-xs focus:ring-2 focus:ring-blue-600 focus:outline-none">
