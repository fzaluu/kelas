...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <a href="{{ route('development.public.documents.create') }}" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-bold transition shrink-0 self-start sm:self-auto">
            TAMBAH DOKUMEN BARU
        </a>
    </div>
            ...
            <tbody class="divide-y divide-slate-100 text-xs text-slate-700">
                @forelse($documents as $item)
                    <tr class="hover:bg-slate-50/50 transition">
                        ...
                        <td class="py-3 px-4">
                            <span class="px-2 py-0.5 rounded-full text-[10px] font-bold {{ $item->status === 'ACTIVE' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-slate-100 text-slate-500 border border-slate-200' }}">
                                {{ $item->status }}
                            </span>
                        </td>
