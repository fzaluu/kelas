...
<?php $__env->startSection('title', 'Kelola Dokumentasi - XI PPLG 2'); ?>
...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <a href="<?php echo e(route('development.public.documents.create')); ?>" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-bold transition shrink-0">
            + Tambah Dokumen Baru 📄
        </a>
    </div>
    ...
    <div class="flex items-center space-x-2 border-b border-slate-200 overflow-x-auto pb-2">
        <a href="<?php echo e(route('development.public.announcements.index')); ?>" 
           class="px-4 py-2 rounded-xl text-xs font-bold <?php echo e(request()->routeIs('development.public.announcements.*') ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-100'); ?>">
            📢 Pengumuman
        </a>
        
        <a href="<?php echo e(route('development.public.agendas.index')); ?>" 
           class="px-4 py-2 rounded-xl text-xs font-bold <?php echo e(request()->routeIs('development.public.agendas.*') ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-100'); ?>">
            📅 Agenda
        </a>
        
        <a href="<?php echo e(route('development.public.documents.index')); ?>" 
           class="px-4 py-2 rounded-xl text-xs font-bold <?php echo e(request()->routeIs('development.public.documents.*') ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-100'); ?>">
            📄 Dokumentasi
        </a>
        
        <a href="<?php echo e(route('development.public.galleries.index')); ?>" 
            class="px-4 py-2 rounded-xl text-xs font-bold <?php echo e(request()->routeIs('development.public.galleries.*') ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-100'); ?>">
                🖼️ Galeri Kegiatan
        </a>
                ...
                <?php $__empty_1 = true; $__currentLoopData = $documents; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr class="hover:bg-slate-50/50 transition">
                        ...
                        <td class="py-3 px-4 font-mono text-[11px] text-blue-600">
                            📁 <?php echo e($item->mediaFile->original_name ?? '-'); ?>

                        </td>
                        <td class="py-3 px-4">
                            <span class="px-2 py-0.5 rounded-full text-[10px] font-bold <?php echo e($item->status === 'ACTIVE' ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500'); ?>">
                                <?php echo e($item->status); ?>

                            </span>
                        </td>
                        <td class="py-3 px-4 text-right">
                            <form action="<?php echo e(route('development.public.documents.destroy', $item->id)); ?>" method="POST" onsubmit="return confirm('Hapus dokumen ini?')">
                                <?php echo csrf_field(); ?>
    ...
    <div><?php echo e($documents->links()); ?></div>
</div>
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\website-pplg\resources\views/pages/development/public/documents/index.blade.php ENDPATH**/ ?>