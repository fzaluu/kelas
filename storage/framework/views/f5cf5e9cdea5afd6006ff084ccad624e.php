...
<?php $__env->startSection('title', 'Jadwal Pelajaran & Piket - XI PPLG 2'); ?>
            ...
            <div class="bg-white rounded-2xl border border-slate-200/80 p-4 shadow-sm space-y-4 flex flex-col justify-between">
                <div class="space-y-3">
                    <div class="flex items-center justify-between border-b border-slate-100 pb-2.5">
                        <h3 class="text-sm font-extrabold text-slate-900 tracking-wider uppercase"><?php echo e($day); ?></h3>
                        <span class="text-[10px] font-bold text-blue-700 bg-blue-50 px-2 py-0.5 rounded-md border border-blue-200">
                            <?php echo e(isset($schedules[$day]) ? count($schedules[$day]) : 0); ?> Mapel
                        </span>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <?php $__empty_1 = true; $__currentLoopData = $schedules[$day] ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <div class="p-2.5 rounded-xl bg-slate-50 border border-slate-200/60 flex items-center justify-between">
                                <div class="space-y-0.5 min-w-0 flex-1 pr-2">
                                    <p class="text-xs font-bold text-slate-800 truncate"><?php echo e($item->subject_name); ?></p>
                                    <p class="text-[10px] text-slate-500 truncate"><?php echo e($item->teacher_name ?? 'Pengajar -'); ?> | <?php echo e($item->room); ?></p>
                                    <p class="text-[10px] font-mono font-bold text-blue-600"><?php echo e($item->start_time); ?> - <?php echo e($item->end_time); ?></p>
                                </div>
                                <form action="<?php echo e(route('development.academic.schedules.destroy', $item->id)); ?>" method="POST" onsubmit="return confirm('Hapus mapel ini?')">
                                    <?php echo csrf_field(); ?>
                    ...
                    <div class="space-y-2 pt-2 border-t border-slate-100">
                        ...
                        <div class="flex flex-wrap gap-1.5">
                            <?php $__empty_1 = true; $__currentLoopData = $pikets[$day] ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <span class="inline-flex items-center space-x-1 px-2.5 py-1 rounded-lg text-[10px] font-bold bg-slate-100 text-slate-700 border border-slate-200/60">
                                    <span><?php echo e($p->student_name); ?></span>
                                    <form action="<?php echo e(route('development.academic.piket.destroy', $p->id)); ?>" method="POST" class="inline">
                                        <?php echo csrf_field(); ?>
                                        <?php echo method_field('DELETE'); ?>
...
<div id="modalSchedule" class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl p-6 max-w-md w-full space-y-4 shadow-xl relative">
        ...
        <form action="<?php echo e(route('development.academic.schedules.store')); ?>" method="POST" class="space-y-3">
            <?php echo csrf_field(); ?>
            <div>
...
<div id="modalPiket" class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl p-6 max-w-sm w-full space-y-4 shadow-xl relative">
        ...
        <form action="<?php echo e(route('development.academic.piket.store')); ?>" method="POST" class="space-y-3">
            <?php echo csrf_field(); ?>
            <div>
                <label class="block text-[10px] font-bold text-slate-700 uppercase">HARI *</label>
                <select name="day" required class="w-full px-3 py-2 rounded-xl border border-slate-200 text-xs focus:outline-none focus:border-blue-600 bg-white">
...
<?php echo $__env->make('layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\website-pplg\resources\views/pages/development/academic/schedules/index.blade.php ENDPATH**/ ?>