...
<head>
    ...
    <title><?php echo $__env->yieldContent('title', 'Dashboard'); ?> - Portal XI PPLG 2</title>
...
<body class="bg-slate-100 min-h-screen font-sans antialiased text-slate-800" x-data="{ mobileDrawer: false }">
         ...
         <aside class="w-64 bg-slate-900 text-slate-300 flex-col justify-between hidden md:flex shrink-0 border-r border-slate-800 sticky top-0 h-screen overflow-y-auto">
            <div>
                <div class="p-6 border-b border-slate-800 flex items-center space-x-3">
                    ...
                    <div>
                        ...
                        <span class="text-[10px] px-2 py-0.5 rounded-full font-bold bg-blue-900/60 text-blue-300 uppercase tracking-wider border border-blue-700/50">
                            <?php echo e(Auth::user()->role ?? 'Development'); ?>
                ...
                <nav class="p-4 space-y-1.5 text-xs">
                    <?php echo $__env->make('partials.sidebar-menu', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                </nav>
            </div>

            <div class="p-4 border-t border-slate-800">
                <a href="<?php echo e(route('home')); ?>" class="flex items-center justify-center space-x-2 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-xs font-semibold text-slate-300 transition">
                    <span>🌐</span>
        ...
        <div class="flex-1 flex flex-col min-w-0">
            ...
            <header class="h-16 bg-white border-b border-slate-200/80 px-4 sm:px-6 flex items-center justify-between sticky top-0 z-20 shadow-sm">
                <div class="flex items-center space-x-3">
                    ...
                    <h1 class="text-sm font-bold text-slate-800"><?php echo $__env->yieldContent('title', 'Dashboard'); ?></h1>
                </div>

                <div class="flex items-center space-x-3">
                    <div class="text-right hidden sm:block">
                        <p class="text-xs font-bold text-slate-900 leading-none"><?php echo e(Auth::user()->name ?? Auth::user()->username); ?></p>
    ...
    <nav class="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-white border-t border-slate-200 flex items-center justify-around z-30 shadow-lg px-2">
        <a href="<?php echo e(route('development.dashboard')); ?>" class="flex flex-col items-center text-slate-500 hover:text-blue-600 space-y-0.5">
            <span class="text-lg">🏠</span>
            <span class="text-[10px] font-semibold">Home</span>
        </a>
        <a href="<?php echo e(route('development.users.index')); ?>" class="flex flex-col items-center text-slate-500 hover:text-blue-600 space-y-0.5">
            <span class="text-lg">👥</span>
            <span class="text-[10px] font-semibold">Users</span>
        </a>
        <div class="relative -top-3">
            <a href="<?php echo e(route('development.users.create')); ?>" class="w-13 h-13 rounded-full bg-gradient-to-tr from-blue-700 to-blue-500 text-white font-bold flex items-center justify-center shadow-lg shadow-blue-600/40 border-4 border-slate-100 active:scale-95 transition">
    ...
    <div x-show="mobileDrawer" x-cloak class="fixed inset-0 z-50 md:hidden flex">
        ...
        <div x-show="mobileDrawer" class="relative max-w-xs w-full bg-slate-900 text-slate-300 p-6 flex flex-col justify-between space-y-6 shadow-2xl">
            ...
            <div class="pt-4 border-t border-slate-800">
                <a href="<?php echo e(route('home')); ?>" class="flex items-center justify-center space-x-2 py-2.5 rounded-xl bg-slate-800 text-xs font-semibold text-slate-300">
                    <span>🌐</span>
                    <span>Lihat Web Publik</span>
                </a>
            </div>
    ...
    <div id="globalToast" 
         class="fixed top-20 right-6 z-[9999] max-w-sm w-full bg-white rounded-2xl shadow-2xl border-2 p-4 flex items-center space-x-3 transition-all duration-300 transform translate-y-0 opacity-100 <?php echo e(session('success') ? 'border-emerald-500/40 text-emerald-900 shadow-emerald-500/10' : 'border-rose-500/40 text-rose-900 shadow-rose-500/10'); ?>">
        
        <!-- Icon Status -->
        <div class="flex-shrink-0 w-9 h-9 rounded-xl flex items-center justify-center font-bold text-base shadow-sm <?php echo e(session('success') ? 'bg-emerald-100 text-emerald-600' : 'bg-rose-100 text-rose-600'); ?>">
...
</html><?php /**PATH C:\laragon\www\website-pplg\resources\views/layouts/dashboard.blade.php ENDPATH**/ ?>